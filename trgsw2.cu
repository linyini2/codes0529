...
// 以下是采用了一维数组的计算
__global__ void TLU_GPU_1(uint32_t *d_database, uint32_t *d_params, uint32_t *d_decbit, cuDoubleComplex *d_twist, int cnt, uint32_t *d_t1, cuDoubleComplex *d_decvecfft)
{
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ void TLU_GPU_1(uint32_t *d_params, cuDoubleComplex *d_twist, int cnt, uint32_t *d_t1, cuDoubleComplex *d_decvecfft)
{
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ void TLU_GPU_2(cuDoubleComplex *id, uint32_t *d_params, cuDoubleComplex *d_twist, int cnt, cuDoubleComplex *d_decvecfft, cuDoubleComplex *d_t4)
{
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
    int M = d_params[3] / 2;
...
__global__ void TLU_GPU_3(uint32_t *d_database, uint32_t *d_params, cuDoubleComplex *d_twist, cuDoubleComplex *d_t5, int threadnum, uint32_t *d_t6, int cnt)
{
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
    int M = d_params[3] / 2;
...
void Test_STLU_2()
{
    ...
    for (int cnt = 0; cnt < dlen; cnt++)
    {
        ...
        if (threadnum > 1024)
        {
            TLU_GPU_1<<<threadnum / 1024, 1024>>>(d_database, d_params, d_decbit, d_twist, cnt, d_t1, d_decvecfft);
            cudaDeviceSynchronize();
            TLU_GPU_1<<<threadnum / 1024, 1024>>>(d_params, d_twist, cnt, d_t1,d_decvecfft);
            cudaDeviceSynchronize();
        }
        else{
            TLU_GPU_1<<<1, threadnum>>>(d_database, d_params, d_decbit, d_twist, cnt, d_t1, d_decvecfft);
            cudaDeviceSynchronize();
            TLU_GPU_1<<<1, threadnum>>>(d_params, d_twist, cnt, d_t1,d_decvecfft);
            cudaDeviceSynchronize();
        }
        ...
        // cudaMemcpy(h_decvecfft, d_decvecfft, NXY * sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost);
        ...
        // cuDoubleComplex *d_newid;
        // CHECK(cudaMalloc((void **)&d_newid, lines * p128.N * sizeof(cuDoubleComplex)));
        // CHECK(cudaMemcpy(d_newid, id[cnt], lines * p128.N * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice));

        if(threadnum > 1024)
        {
            TLU_GPU_2<<<threadnum / 1024, 1024>>>(d_id+cnt*idsize, d_params, d_twist, cnt, d_decvecfft, d_t4);
            cudaDeviceSynchronize();
        }
        else{
            TLU_GPU_2<<<1, threadnum>>>(d_id+cnt*idsize, d_params, d_twist, cnt, d_decvecfft, d_t4);
            cudaDeviceSynchronize();
        }
        if(threadnum / 2 > 1024)
        {
        ...
        if(threadnum / 2 > 1024)
        {
            TLU_GPU_3<<<threadnum / 2048, 1024>>>(d_database, d_params, d_twist, d_t5, threadnum / 2, d_t6, cnt);
            CHECK(cudaDeviceSynchronize());
            // TLU_GPU_3<<<threadnum / 2048, 1024>>>(d_params, d_t5, d_t6);
            // CHECK(cudaDeviceSynchronize());
        }
        else{
            TLU_GPU_3<<<1, threadnum / 2>>>(d_database, d_params, d_twist, d_t5, threadnum / 2, d_t6, cnt);
            CHECK(cudaDeviceSynchronize());
...
int main()
{
    // TestTest_STLU_1 使用了统一内存分配和多维数组，N 和 dlen 在不超过内存限制的情况下，解密结果百分百正确
    // Test_STLU_1();

    // TestTest_STLU_2 使用了一维数组，解密结果有时不正确（把N 和 dlen调大会不正确），估计是线程和索引计算出了问题
    // 一般 N = 1024， dlen = 16, flen 对应的就是 2^16 条数据
    Test_STLU_2();
