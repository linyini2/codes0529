...
__global__ void TLU_GPU_2(cuDoubleComplex *id, uint32_t *d_params, cuDoubleComplex *d_twist, int cnt, cuDoubleComplex *d_decvecfft, cuDoubleComplex *d_t4, cuDoubleComplex *d_t5, int threadnum)
{
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
    int M = d_params[3] / 2;
...
void Test_STLU_2()
{
    ...
    Params128 p128 = Params128(128, 630, 2, 10, 9, pow(2.0, -15.4), pow(2.0, -28), 2);
    // 数据库一共有 flen 条数据，每条数据的序号用 dlen 个二进制位来表示
    int dlen = 6;
    int flen = 1 << dlen;
    int M = p128.N / 2;
    ...
    for (int cnt = 0; cnt < dlen; cnt++)
    {
        ...
        if(2 * threadnum > 1024)
        {
            TLU_GPU_2<<<threadnum / 512, 1024>>>(d_id+cnt*idsize, d_params, d_twist, cnt, d_decvecfft, d_t4, d_t5, threadnum * 2);
            cudaDeviceSynchronize();
        }
        else{
            TLU_GPU_2<<<1, threadnum * 2>>>(d_id+cnt*idsize, d_params, d_twist, cnt, d_decvecfft, d_t4, d_t5, threadnum * 2);
            cudaDeviceSynchronize();
        }
